...
function toSticker(buf, packName, authName) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (!packName) packName = global.config.packName
            if (!authName) authName = global.config.authName
            ...
            await ffmpeg(input)
                ...
                .on('start', function (cmd) {
                    console.log(color('[ SYSTEM ]', 'yellow'), color('Start making stickers'));
                })
                .on('error', function (err) {
                    console.log(color('[ SYSTEM ]', 'red'), color('Failed to make a sticker'));
                ...
                .on('end', function () {
                    console.log(color('[ SYSTEM ]', 'yellow'), color('Success in making stickers'));
...
function webpToPng(buf) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            console.log(color('[ SYSTEM ]', 'yellow'), color('Start Convert Webp To Png'));
            exec(`ffmpeg -i ./tmp/${name}.webp ./tmp/${name}.png`, (err) => {
                if (err) {
                    console.log(color('[ SYSTEM ]', 'red'), color('Failed Convert Webp To Png'));
                    if (fs.existsSync(input)) fs.unlinkSync(input)
                    if (fs.existsSync(output)) fs.unlinkSync(output)
                    return reject(err)
                }
                console.log(color('[ SYSTEM ]', 'yellow'), color('Success Convert Webp To Png'));
                let hasil = fs.readFileSync(output)
                fs.unlinkSync(input)
                fs.unlinkSync(output)
                resolve(hasil)
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            await ffmpeg(input)
                .format('mp3')
                .on('start', function (cmd) {
                    console.log(color('[ SYSTEM ]', 'yellow'), color('Start Convert Video To Audio'));
                })
                .on('error', function (err) {
                    console.log(color('[ SYSTEM ]', 'red'), color('Failed Convert Video To Audio'));
                    if (fs.existsSync(input)) fs.unlinkSync(input)
                    if (fs.existsSync(output)) fs.unlinkSync(output)
                    resolve(buf)
                })
                .on('end', function () {
                    console.log(color('[ SYSTEM ]', 'yellow'), color('Success Convert Video To Audio'));
                    let hasil = fs.readFileSync(output)
                    if (fs.existsSync(input)) fs.unlinkSync(input)
                    if (fs.existsSync(output)) fs.unlinkSync(output)
                    resolve(hasil)
